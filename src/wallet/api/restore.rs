	...
	K: Keychain,
{
	...
	for output in outputs.iter() {
		...
		if switch != SwitchCommitmentType::Regular {
			println!("Unexpected switch commitment type {:?}", switch);
		}
	...
	K: Keychain,
{
	...
	//let wallet_outputs = updater::retrieve_outputs(wallet, true, None, None, 0, 0)?;
        let wallet_outputs = wallet.outputs().collect::<Vec<_>>();
        let wo_with_ped = updater::retrieve_outputs(wallet, true, None, None, 0, 0)?;
	...
	for deffo in chain_outs.into_iter() {
		let matched_out = wo_with_ped.iter().find(|wo| wo.1 == deffo.commit);
		match matched_out {
			Some(s) => {
				if s.0.status == OutputStatus::Spent {
					accidental_spend_outs.push((s.clone(), deffo.clone()));
				}
	...
	for m in accidental_spend_outs.into_iter() {
		...
		println!(
			...
			o.0.value, o.0.key_id, m.1.commit,
		);
		o.0.status = OutputStatus::Unspent;
		// any transactions associated with this should be cancelled
		cancel_tx_log_entry(wallet, &o.0)?;
		let mut batch = wallet.batch()?;
		batch.save_output(&o.0)?;
		batch.commit()?;
	}
	...
	if delete_unconfirmed {
		...
		let unconfirmed_outs: Vec<&OutputData> = wallet_outputs
			.iter()
...
/// assume wallet contents have been freshly updated with contents
/// of latest block
pub fn scan_outputs<T, C, K>(wallet: &mut T, pub_keys: Vec<PublicKey>, output_fn : String ) -> Result<(), Error>
	where
		T: WalletBackend<C, K>,
		C: NodeClient,
		K: Keychain,
	...
	loop {
		...
		for output in outputs.iter() {
			...
			for pk_info in &pub_keys_info {
				...
				result_file.write_fmt( format_args!("PublicKey={} Commit={} amount={} height={} mmr_index={}\n", pk_info.pub_key_hex, commit.to_hex(), info.value, height, mmr_index) )?;
