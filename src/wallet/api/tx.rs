    ...
    K: Keychain,
{
    ...
    let tx_vec = updater::retrieve_txs(wallet, tx_id, tx_slate_id, Some(&parent_key_id), false, 0, 0)?;
    if tx_vec.len() != 1 {
...
{
    // finalize command
    let tx_vec = updater::retrieve_txs(wallet, None, Some(slate.id), None, false, 0, 0)?;
    let mut tx = None;
